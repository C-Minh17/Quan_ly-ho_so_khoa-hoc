...
export const OIDCBounder_: FC<{ children: React.ReactElement }> = ({ children }) => {
	...
	// const { handleGetUserMe } = useModel('smc-user.smc-user');
	const hasRequestedUserMeRef = useRef(false);
	const auth = useAuth();
	...
	const handleLogin = async () => {
		if (auth.user) {
			...
			try {
				...
				// [TEMP] Comment role checking - bỏ comment khi Keycloak cấu hình đúng
				...
				if (!hasRequestedUserMeRef.current) {
					hasRequestedUserMeRef.current = true;
					// void handleGetUserMe();
				}
			} catch {
