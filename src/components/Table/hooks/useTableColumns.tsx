...
export const useTableColumns = ({ columns, sort, addStt, dsPhanVung }: UseTableColumnsProps) => {
	...
	const handleFilter = (
		...
		operator: EOperatorType = EOperatorType.INCLUDE,
	) => {
		if (!values || !values.length) {
			...
			if (filter)
				...
				tempFilters = tempFilters.map((item) =>
					JSON.stringify(item.field) === JSON.stringify(dataIndex)
						? { ...item, active: true, operator, values, readOnly }
						: item,
				);
			// Thêm quy tắc lọc mới cho cột này
			else
	...
	const getColumnDateProps = (dataIndex: any, showTime?: boolean): Partial<IColumn<unknown>> => {
		return {
			filterDropdown: ({ setSelectedKeys, selectedKeys, confirm }) => {
				return (
					<div className='column-search-box' onKeyDown={(e) => e.stopPropagation()}>
						...
						<div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
							<Button
								...
								size='small'
								onClick={() => {
									handleFilter(dataIndex, selectedKeys as string[], EOperatorType.BETWEEN);
									confirm();
								}}
								style={{ flex: 1 }}
							>
							...
							<Button
								...
								onClick={() => {
									setSelectedKeys([]);
									handleFilter(dataIndex, [], EOperatorType.BETWEEN);
									confirm();
								}}
								style={{ flex: 1 }}
							>
			...
			filterIcon: () => {
				const values = getFilterColumn(dataIndex, EOperatorType.BETWEEN, true)?.values;
				const filtered = values && values.length > 0;
				return <CalendarOutlined className={filtered ? 'text-primary' : undefined} />;
			},
		};
	};
			...
			...(item.filterType === 'string'
				...
				: item.filterType === 'select'
					...
					: item.filterType === 'customselect'
						...
						: item.filterType === 'date' || item.filterType === 'datetime'
							? getColumnDateProps(item.dataIndex, item.filterType === 'datetime')
							: undefined),
				...
				...(child.filterType === 'string'
					...
					: child.filterType === 'select'
						...
						: child.filterType === 'customselect'
							...
							: child.filterType === 'date' || child.filterType === 'datetime'
								? getColumnDateProps(child.dataIndex, child.filterType === 'datetime')
								: undefined),
			})),
		}));
